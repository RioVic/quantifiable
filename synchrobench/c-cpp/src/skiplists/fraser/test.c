...
void *test(void *data) {
    ...
    while (1) {
		...
		switch (op->type) {
			case ADD:
			op->result = sl_add_old(d->set, op->arg);
			if (op->result) d->nb_added++;
			d->nb_add++;
			break;
			case DELETE:
			op->result = sl_remove_old(d->set, op->arg);
			if (op->result) d->nb_removed++;
			d->nb_remove++;
			break;
			case READ:
			op->result = sl_contains_old(d->set, op->arg);
...
void replay(thread_data_t *d) {
	...
	switch (op->type) {
	    case ADD:
		op->replay_result = sl_add_old(d->set, op->arg);
		break;
	    case DELETE:
		op->replay_result = sl_remove_old(d->set, op->arg);
		break;
	    case READ:
		op->replay_result = sl_contains_old(d->set, op->arg);
		break;
	    default:
		assert(0);
	} 
